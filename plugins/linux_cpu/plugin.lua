...
local function collect()
  for line in io.lines(HOST_PROC_DIR .. "/stat") do
    ...
    if processes then
      ...
      manager:send_metric({ plugin = plugin .. "." .. key, int = tonumber(val) })
    end
    ...
    if ctxt then
      ...
      if diff then manager:send_metric({ plugin .. ".ctxt", float = diff }) end
    end
    ...
    if processes then
      ...
      if diff then manager:send_metric({ plugin .. ".fork_rate", float = diff }) end
    end
    ...
    if intr then
      ...
      if diff then manager:send_metric({ plugin .. ".intr", float = diff }) end
    end
